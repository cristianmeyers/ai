...
```bash
nano ~/docker-apps/open-webui/docker-compose.yml
```
...
> **Note :** Le tout premier compte enregistré sur l'interface devient automatiquement le compte **Administrateur**. C'est depuis ce compte que vous pourrez gérer les accès des futurs utilisateurs si nécessaire.
...
| **Redémarrer le conteneur** | `docker compose restart open-webui`           |
